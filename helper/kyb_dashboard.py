...
class KYBDashboard:
    ...
    def calculate_kpis(self)-> dict:
        ...
        total_apps = self.df['Case_ID'].nunique()
        manual_cases = self.df[self.df['Processing_Mode'] == 'Manual']['Case_ID'].unique()
        stp_cases_count = total_apps - len(manual_cases)
        stp_rate = (stp_cases_count / total_apps) * 100 if total_apps > 0 else 0

        avg_auto_time = self.df[self.df['Processing_Mode'] == 'Automated']['Duration_Min'].mean()
        avg_manual_time = self.df[self.df['Processing_Mode'] == 'Manual']['Duration_Min'].mean()
